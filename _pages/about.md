...
**데이터 엔지니어링 및 풀스택 개발 (Classic ASP)** | 2018.06 ~ 2018.12

호주 정부 승인을 받은 합법 스포츠 베팅 업체에서 실시간 배당 데이터 수집 엔진 및 UI 개발 담당

**기여:**
- **실시간 경기 데이터 파싱 엔진 구축:** OddsPortal 등 외부 소스의 실시간 배당 정보를 수집하는 스크래핑 엔진을 Classic ASP(VBScript)로 단독 구축
- **대규모 데이터 예외 처리:** 소스 사이트의 비정기적인 DOM 구조 변경 및 종목별(축구, 농구 등) 상이한 데이터 스키마에 대응하기 위한 1,000라인 이상의 Exception Handling 로직 구현
- **데이터 정규화(Normalization):** 서로 다른 소스 간의 팀명(예: Man Utd vs Manchester United) 및 리그명 불일치 해소를 위한 매핑 엔진 설계 및 DB 동기화
- **실시간 대시보드 UI/UX:** 수집된 데이터를 실시간으로 시각화하여 사용자에게 배당 흐름을 보여주는 대시보드 디자인 및 프론트엔드 개발

**성과:**
- **데이터 유실률 1% 미만 유지:** 강력한 예외 처리 로직을 통해 소스 사이트의 구조 변경에도 중단 없는 데이터 수집 달성
- **풀스택 프로세스 경험:** 원시 데이터 수집부터 DB 스키마 설계, 웹 렌더링까지 이어지는 서비스 전체 파이프라인 단독 경험
- **레거시 최적화:** 프레임워크가 없는 환경에서 리소스 관리 최적화를 통해 실시간 데이터 처리 성능 확보

**기술**: Classic ASP, VBScript, MS-SQL, JavaScript, CSS (Table-based Design)
