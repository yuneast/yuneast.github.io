...
- 컨트롤러에 집중된 로직을 Service-Repository 패턴으로 리팩토링
- 배포 전 검증용 목 데이터 생성, 트랜잭션 롤백으로 실제 DB 반영 방지
...
**데이터 엔지니어링** (2018.06 ~ 2018.12)

호주 정부 승인 합법 스포츠 베팅 업체, OddsPortal 실시간 배당 데이터 파싱 엔진 단독 개발

**기여:**
- Classic ASP 기반 OddsPortal 실시간 배당 데이터 스크래핑 엔진 구축
- 소스 사이트 DOM 구조 변경, 종목별(축구, 농구 등) 상이한 데이터 스키마 대응 예외 처리 로직 구현
- 팀명, 리그명 불일치 해소 매핑 엔진 설계 및 DB 동기화

**성과:**
- 예외 처리 로직으로 소스 사이트 구조 변경 시에도 데이터 유실률 1% 미만 유지
- 실시간 배당 데이터 수집, DB 동기화 파이프라인 단독 구축

**기술**: Classic ASP, VBScript, MS-SQL
