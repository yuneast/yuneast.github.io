...
스타트업 초기에 중요한 건 **속도**다. 3개월 안에 MVP를 만들어서 시장 검증을 받아야 한다.

**우선순위:**
1. 빠른 개발 속도 (가장 중요)
2. 주니어 온보딩 용이성
3. 유지보수성
4. 성능 (당장은 덜 중요, 나중에 최적화 가능)
...
FastAPI는 Python만 알면 시작할 수 있다. 곧 합류할 주니어 개발자들 온보딩이 쉬울 거라고 판단했다.

실제로 주니어 개발자가 첫날 CRUD API를 만들었다. Django DRF보다 배우기 쉬웠다.
...
1. **개발 속도가 빨랐다.**
   - 3개월 만에 MVP 출시
   - 관리비 대납, 주차 관리, 투표 기능 모두 구현

2. **주니어 온보딩이 쉬웠다.**
   - 주니어 백엔드 개발자가 1주일 만에 기능 개발 시작
...
이유:
- 빠른 개발 속도가 초기 스타트업에 가장 중요했다
- 단점(ORM, Admin)은 해결 가능했다
- MVP 출시 후 시장 검증을 빨리 받을 수 있었다
...
FastAPI는 우리 팀과 잘 맞았다. 빠른 개발 속도로 3개월 만에 MVP를 만들고, 시장 검증을 받을 수 있었다. 단점도 있었지만 장점이 훨씬 컸다. 스타트업 초기라면 FastAPI를 추천한다.
